...
import { LayoutDashboard, UserPlus, Users, CalendarCheck, PhoneCall, MessageCircle, Receipt, BarChart3, Upload, Activity, Workflow, HeartHandshake, Stethoscope, ChevronLeft, ChevronRight, Search, Bell, UserRound, Menu, X } from "lucide-react";
...
import Treatment from "./views/Treatment.jsx";

const NAV = [
  { key: "dashboard", label: "Dashboard", icon: LayoutDashboard },
  { key: "pipeline", label: "Conversion Analysis", icon: Workflow },
  { key: "reports", label: "Reports & MIS", icon: BarChart3 },
  { key: "leads", label: "Leads", icon: UserPlus },
  { key: "import", label: "Import leads", icon: Upload },
  { key: "patients", label: "Patients", icon: Users },
  { key: "appointments", label: "Appointments", icon: CalendarCheck },
  { key: "counselling", label: "Counselling", icon: HeartHandshake },
  { key: "treatment", label: "Treatment & Care", icon: Stethoscope },
...
const GLOBAL_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  @media print { aside, header.topbar, .no-print { display: none !important; } .reports-layout { display: block !important; } main { padding: 0 !important; } }
`;

function TopBar({ data, onNavigate, onOpenPatient, dueCount, isMobile, onMenu }) {
  ...
  return (
    <header className="topbar" style={{ height: isMobile ? 64 : 76, background: "#fff", borderBottom: `1px solid ${COLORS.line}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "0 12px" : "0 28px", gap: isMobile ? 8 : 16, position: "sticky", top: 0, zIndex: 20 }}>
      {isMobile && (
        <button className="icon-btn" onClick={onMenu} aria-label="Open menu" style={{ border: "none", background: "transparent", cursor: "pointer", padding: 8, borderRadius: 10, color: COLORS.ink, display: "inline-flex", flexShrink: 0 }}>
          <Menu size={23} />
        </button>
      )}
      <div ref={boxRef} style={{ position: "relative", width: "100%", maxWidth: 400, minWidth: 0 }}>
        ...
        <input
          ...
          style={{ width: "100%", height: isMobile ? 42 : 46, padding: "0 12px 0 40px", borderRadius: 10, border: `1px solid ${COLORS.line}`, background: COLORS.cream, fontSize: 14, fontFamily: FONT_SANS, color: COLORS.ink, outline: "none" }}
        />
        {open && q.trim() && (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 4 : 14, flexShrink: 0 }}>
        ...
        <div title="Front Desk, Admin" style={{ display: "flex", alignItems: "center", gap: 11, background: isMobile ? "transparent" : COLORS.cream, borderRadius: 12, padding: isMobile ? 2 : "8px 16px 8px 9px" }}>
...
function Sidebar({ view, setView, collapsed: collapsedProp, setCollapsed, isMobile, open, onClose }) {
  const collapsed = isMobile ? false : collapsedProp;
  const mobileStyle = isMobile
    ? { position: "fixed", left: 0, top: 0, bottom: 0, height: "100dvh", zIndex: 60, transform: open ? "translateX(0)" : "translateX(-100%)", transition: "transform .22s ease", boxShadow: open ? "0 0 40px rgba(0,0,0,.35)" : "none", visibility: open ? "visible" : "hidden" }
    : { position: "sticky", top: 0, height: "100vh", transition: "width .2s ease" };
  return (
    <aside aria-hidden={isMobile && !open ? true : undefined} style={{ width: collapsed ? 78 : isMobile ? "min(290px, 84vw)" : 272, flexShrink: 0, background: COLORS.navy, color: COLORS.navyText, display: "flex", flexDirection: "column", ...mobileStyle }}>
      <div style={{ height: 76, display: "flex", alignItems: "center", gap: 12, padding: collapsed ? "0 19px" : "0 20px", borderBottom: `1px solid ${COLORS.navyLine}` }}>
        ...
        {isMobile && (
          <button className="icon-btn" onClick={onClose} aria-label="Close menu" style={{ marginLeft: "auto", border: "none", background: "transparent", color: COLORS.navyText, cursor: "pointer", padding: 6, borderRadius: 8, display: "inline-flex" }}>
            <X size={21} />
          </button>
        )}
      </div>
      ...
      <div style={{ borderTop: `1px solid ${COLORS.navyLine}`, padding: "12px 14px", display: isMobile ? "none" : "block" }}>
...
export default function App() {
  ...
  const openPatient = (id) => { setMenuOpen(false); window.scrollTo(0, 0); setFocusPatientId(id); setView("patients"); };

  return (
    <div style={{ fontFamily: FONT_SANS, background: COLORS.cream, minHeight: "100vh", display: "flex" }}>
      <style>{GLOBAL_CSS}</style>
      <Sidebar view={view} setView={go} collapsed={collapsed} setCollapsed={setCollapsed} isMobile={isMobile} open={menuOpen} onClose={() => setMenuOpen(false)} />
      {isMobile && menuOpen && <div onClick={() => setMenuOpen(false)} aria-hidden="true" style={{ position: "fixed", inset: 0, background: "rgba(16,38,46,0.45)", zIndex: 55 }} />}
      <div style={{ flex: 1, minWidth: 0 }}>
        <TopBar data={data} onNavigate={go} onOpenPatient={openPatient} dueCount={dueCount} isMobile={isMobile} onMenu={() => setMenuOpen(true)} />
        <main style={{ padding: isMobile ? "20px 14px 40px" : "32px 30px 48px" }}>
          {view === "dashboard" && <Dashboard data={data} onNavigate={go} />}
          {view === "pipeline" && <Pipeline data={data} onOpenPatient={openPatient} onNavigate={go} />}
          {view === "treatment" && <Treatment data={data} setData={setData} onOpenPatient={openPatient} />}
          {view === "counselling" && <Counselling data={data} setData={setData} onOpenPatient={openPatient} />}
          {view === "leads" && <Leads data={data} setData={setData} onOpenPatient={openPatient} onNavigate={go} />}
