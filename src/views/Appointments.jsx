...
export default function Appointments({ data, setData, onOpenPatient, initialTab = "opd" }) {
  ...
  return (
    <div>
      <SectionTitle
        title="Appointments"
        subtitle="OPD bookings, referrals to the counsellor, and IPD / surgery conversion"
        action={tab === "opd" && <Btn kind="primary" icon={Plus} onClick={() => setShowAdd((s) => !s)}>Book appointment</Btn>}
      />

      <div role="tablist" style={{ display: "flex", gap: 4, borderBottom: `1px solid ${COLORS.line}`, marginBottom: 24 }}>
        {[["opd", "OPD Appointments", Stethoscope, data.appointments.length], ["surgery", "IPD / Surgery Conversion", Scissors, data.surgeries.length]].map(([k, label, Icon, n]) => {
          const on = tab === k;
          return (
            <button key={k} role="tab" aria-selected={on} onClick={() => setTab(k)} style={{ fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 8, padding: "11px 16px", marginBottom: -1, border: "none", borderBottom: `2px solid ${on ? COLORS.blue : "transparent"}`, background: "transparent", cursor: "pointer", fontSize: 14.5, fontWeight: 600, color: on ? COLORS.blueDeep : COLORS.inkSoft }}>
              <Icon size={17} />
              {label}
