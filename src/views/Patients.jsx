    ...
    <div>
      ...
      {!openPatient && (
        <Card style={{ padding: 0 }}>
          {data.patients.map((p, i) => (
            <div key={p.id} className="row-hover" onClick={() => setOpenId(p.id)} style={{ cursor: "pointer", display: "grid", gridTemplateColumns: isMobile ? "1fr auto" : "1.5fr 0.6fr 1fr 1.2fr auto", gap: 10, alignItems: "center", padding: "14px 22px", borderTop: i ? `1px solid ${COLORS.line}` : "none" }}>
              <div><div style={{ fontSize: 13.5, color: COLORS.ink, fontWeight: 500 }}>{p.name}</div><div style={{ fontSize: 12, color: COLORS.slate }}>{p.mobile || "—"}{isMobile && p.doctor ? `, ${p.doctor}` : ""}</div></div>
              {!isMobile && <div style={{ fontSize: 12.5, color: COLORS.inkSoft }}>{p.age ? `${p.age}, ${p.gender}` : p.gender}</div>}
              {!isMobile && <div style={{ fontSize: 12.5, color: COLORS.inkSoft }}>{p.doctor || "—"}</div>}
              {!isMobile && <Badge tone="slate">{p.department || "General"}</Badge>}
      ...
      {openPatient && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(300px, 1fr) 1.4fr", gap: 16 }}>
