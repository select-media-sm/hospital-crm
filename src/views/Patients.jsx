...
import { uid, todayISO, fmtDate } from "../data.js";
...
export default function Patients({ data, setData }) {
  const [showAdd, setShowAdd] = useState(false);
  const [openId, setOpenId] = useState(null);
  const [form, setForm] = useState({ name: "", age: "", gender: "Female", mobile: "", doctor: "", department: "" });
  ...
  const addPatient = () => {
    ...
    setForm({ name: "", age: "", gender: "Female", mobile: "", doctor: "", department: "" });
    setShowAdd(false);
  };
      ...
      {!openPatient && (
        <Card style={{ padding: 0 }}>
          {data.patients.map((p, i) => (
            <div key={p.id} onClick={() => setOpenId(p.id)} style={{ cursor: "pointer", display: "grid", gridTemplateColumns: "1.5fr 0.6fr 1fr 1.2fr auto", gap: 10, alignItems: "center", padding: "14px 22px", borderTop: i ? `1px solid ${COLORS.line}` : "none" }}>
              ...
              <span style={{ fontSize: 12.5, color: COLORS.blueDeep, fontWeight: 600 }}>View profile →</span>
            </div>
          ))}
