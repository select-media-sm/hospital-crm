...
export default function Counselling({ data, setData, onOpenPatient }) {
  ...
  const createCase = () => {
    ...
    const s = { id: uid(), patientId: caseFor.patientId, patientName: caseFor.patientName, surgeon: caseForm.surgeon || "Unassigned", type: caseForm.type.trim(), stage: "Follow-up Pending", date: "", tentativeMonth: "", notes: "", createdAt: todayISO() };
    ...
    <div>
      ...
      <Modal open={!!caseFor} onClose={() => setCaseFor(null)} title="Open surgery case" subtitle={caseFor ? `${caseFor.patientName}, advised by ${caseFor.doctor || "doctor"}` : ""} width={480}>
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <div style={{ fontSize: 12.5, color: COLORS.inkSoft }}>The case starts as Follow-up Pending under Appointments, in the OPD / Surgery Conversion tab.</div>
