...
import { STAGES, todayISO, daysFromNow, fmtINR } from "../data.js";
...
export default function Dashboard({ data }) {
  ...
  const plannedSurgeries = data.surgeries.filter((s) => s.stage !== "Completed" && s.stage !== "Discharged").length;
  const upcomingSurgeries = data.surgeries.filter((s) => s.date && s.date >= today).length;
  ...
  const completedOPD = data.appointments.filter((a) => a.status === "Completed").length;
  const opdToSurgeryPct = completedOPD ? Math.round((data.surgeries.length / completedOPD) * 100) : 0;
  const bookedOrBeyond = data.leads.filter((l) => l.stage !== "New Lead" && l.stage !== "Contacted" && l.stage !== "Lost").length;
  const leadToApptPct = data.leads.length ? Math.round((bookedOrBeyond / data.leads.length) * 100) : 0;
  ...
  const funnelData = STAGES.lead.filter((s) => s !== "Lost").map((stage) => ({ stage, count: data.leads.filter((l) => l.stage === stage).length }));
        ...
        <ChartCard title="Revenue trend, last 7 days">
          <ResponsiveContainer width="100%" height={190}>
            <LineChart data={trendData} margin={{ left: -10, right: 8, top: 4, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: COLORS.inkSoft }} axisLine={false} tickLine={false} width={40} tickFormatter={(v) => `₹${v >= 1000 ? (v / 1000).toFixed(0) + "k" : v}`} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <ChartCard title="Lead funnel">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={funnelData} margin={{ left: -20, right: 8, top: 4, bottom: 20 }}>
              <CartesianGrid stroke={COLORS.line} vertical={false} />
              <XAxis dataKey="stage" tick={{ fontSize: 9, fill: COLORS.inkSoft }} axisLine={{ stroke: COLORS.line }} tickLine={false} interval={0} angle={-25} textAnchor="end" height={54} />
        ...
        <Card>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, color: COLORS.inkSoft, marginBottom: 7 }}>
              <span>OPD → Surgery</span><span style={{ fontWeight: 600, color: COLORS.ink }}>{opdToSurgeryPct}%</span>
            </div>
            <ProgressBar value={opdToSurgeryPct} tone="green" />
          </div>
          <div style={{ marginBottom: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, color: COLORS.inkSoft, marginBottom: 7 }}>
              <span>Lead → Appointment</span><span style={{ fontWeight: 600, color: COLORS.ink }}>{leadToApptPct}%</span>
            </div>
            <ProgressBar value={leadToApptPct} tone="blue" />
          </div>
