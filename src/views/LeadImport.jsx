...
import { uid, todayISO, normalizeLeadStage } from "../data.js";
import { readSpreadsheetFile, downloadWorkbook } from "../lib/excel.js";
import { Card, Btn, Badge, Select, Field, SectionTitle } from "../components/ui.jsx";

const TARGET_FIELDS = ["name", "mobile", "email", "source", "department", "doctor", "date", "status"];
const KEYWORDS = {
  name: ["name", "patient"],
...
function guessMapping(headers) {
  ...
  TARGET_FIELDS.forEach((field) => {
    const found = headers.find((h) => KEYWORDS[field].some((k) => h.toLowerCase().includes(k)) && !(field === "mobile" && /mail/i.test(h)));
    map[field] = found || "";
  });
  return map;
}
...
export default function LeadImport({ data, setData }) {
  ...
  const downloadSample = () =>
    downloadWorkbook("lead-import-sample.xlsx", [{
      name: "Leads",
      rows: [
        ["Name", "Mobile", "Email", "Source", "Department", "Doctor", "Enquiry date", "Status"],
        ["Rohan Kale", "9876501234", "rohan.kale@gmail.com", "Google Ads", "Orthopaedics", "Dr. Aisha Rao", todayISO(), "New Lead"],
        ["Priti Jadhav", "9823004567", "", "Meta Ads", "Orthopaedics", "", todayISO(), "Follow-up Required"],
      ],
    }]);
  ...
  const doImport = () => {
    ...
    newRows.forEach((r) => {
      ...
      const stage = normalizeLeadStage(r.status) === "Converted" ? "New Lead" : normalizeLeadStage(r.status);
      leads.push({ id: uid(), name: r.name, mobile: r.mobile, email: r.email, lastContacted: null, source: r.source || "Import", department: r.department, doctor: r.doctor, stage, patientId: "", nextFollowup: "", createdAt: r.date || todayISO() });
    });
    ...
    <div>
      ...
      <Card style={{ marginBottom: 18 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <label style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 18px", borderRadius: 10, background: COLORS.blue, color: "#fff", fontSize: 13.5, fontWeight: 600, cursor: "pointer" }}>
            <FileUp size={16} />
            Choose file
            <input type="file" accept=".csv,.xlsx,.xls" onChange={handleFile} style={{ display: "none" }} />
          </label>
          <div style={{ fontSize: 13, color: COLORS.inkSoft, flex: 1, minWidth: 180 }}>{fileName || "No file selected. Accepts .csv, .xlsx, .xls"}</div>
          <Btn icon={Download} onClick={downloadSample}>Download sample file</Btn>
        </div>
        {imported !== null && (
      ...
      {headers.length > 0 && (
        <>
          ...
          <Card style={{ marginBottom: 18 }}>
            ...
            <div style={{ maxHeight: 280, overflow: "auto" }}><div style={{ minWidth: 560 }}>
