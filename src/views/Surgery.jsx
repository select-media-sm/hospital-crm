...
import { STAGES, SURGERY_GROUPS, uid, todayISO, fmtDate, withHistory, makePostopSchedule } from "../data.js";
...
export default function Surgery({ data, setData, onOpenPatient, embedded }) {
  ...
  const addSurgery = () => {
    ...
    const surgery = { id: uid(), patientId: patient.id, patientName: patient.name, surgeon: form.surgeon || patient.doctor || "Unassigned", type: form.type.trim(), stage: form.stage, date: "", tentativeMonth: "", notes: "", createdAt: today };
  ...
  const update = (s, patch) =>
    setData((d) => {
      // Marking a surgery done sets the date to today if it is missing or still in the future, and starts the standard post-op schedule.
      if (patch.stage && SURGERY_GROUPS.done.includes(patch.stage) && (!s.date || s.date > today)) patch = { ...patch, date: today };
      let next = { ...d, surgeries: d.surgeries.map((x) => (x.id === s.id ? { ...x, ...patch } : x)) };
      if (patch.stage && SURGERY_GROUPS.done.includes(patch.stage) && !(d.postop || []).some((e) => e.surgeryId === s.id)) {
        next = { ...next, postop: [...(d.postop || []), ...makePostopSchedule({ ...s, ...patch }, uid)] };
      }
  ...
  return (
    <div>
      {embedded ? (
        ...
        <SectionTitle title="OPD / Surgery Conversion" subtitle="Every advised surgery, from follow-up to booking, or the reason it did not convert" action={<Btn kind="primary" icon={Plus} onClick={() => setShowAdd((s) => !s)}>Add surgery case</Btn>} />
      )}
      {showAdd && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(220px, calc(50% - 10px)), 1fr))", gap: 20, marginBottom: 20 }}>
