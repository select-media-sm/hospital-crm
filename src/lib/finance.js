...
export function unifiedInvoices(data) {
  ...
  const surgery = (data.surgeryBills || []).map((b) => ({
    ...
    patientId: b.patientId,
    patientName: b.patientName,
    // Surgery bills don't store the surgeon, so fall back to the patient's surgery case, then their doctor.
    doctor: b.surgeon || (data.surgeries || []).find((x) => x.patientId === b.patientId)?.surgeon || (data.patients || []).find((p) => p.id === b.patientId)?.doctor || "",
    date: b.date,
    mode: b.mode || "—",
