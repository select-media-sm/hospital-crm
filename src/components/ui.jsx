  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 26, fontWeight: 700, color: COLORS.ink, lineHeight: 1.15, letterSpacing: -0.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{value}</div>
...
export function SectionTitle({ title, subtitle, action }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, marginBottom: 26, flexWrap: "wrap" }}>
      <div>
        <h1 style={{ fontSize: 27, fontWeight: 700, color: COLORS.ink, margin: 0, letterSpacing: -0.4 }}>{title}</h1>
...
export function AddPanel({ children, title }) {
  return (
    <Card style={{ marginBottom: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 14, alignItems: "end" }}>{children}</div>
    </Card>
  );
}
